//reduce (function (acc, item, index)) представляем и решаем для Total
//аккумулятор
//1
// let a = [1, 2, 3, 4, 5];
...
//   console.log("acc", acc);
//   return acc + fruit.quantity; //изза того что это обьекты
// }, 0);
// console.log(totalQuantity); //6 (2+3+1)

//3 нахождение большего числа в массиве, перезаписывает в асс пока не достигнет максимума
...
// }, numbers[0]); //можно не указывать, а написать просто 0(default)
...
//   { names: [], totalAges: 0 }
// );
// console.log(result); //{names: Array(3), totalAges: 77}
...
// const totalBudget = people.reduce(function (acc, person) {
...
const result = people.reduce(
  function (acc, currentValue) {
    acc.totalBudget += currentValue.budget;
    console.log(acc);
